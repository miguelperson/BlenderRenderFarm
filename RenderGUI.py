...
def update_row(row_data):
    ...
    def submit_changes():
        ...
        try:
            conn = connection()
            cursor = conn.cursor()
            cursor.execute(f"UPDATE render SET {updates} WHERE projectID=%s", values)
            conn.commit()
...
def delete_row(projectID):
    if messagebox.askyesno("Confirm Delete", "Are you sure you want to delete this row?"):
        try:
            conn = connection()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM render WHERE projectID = %s", (projectID,))
            conn.commit()
...
def display():
    ...
    conn = connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM render")
